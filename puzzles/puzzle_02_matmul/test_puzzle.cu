...
TEST_CASE(matmul_2x2) {
    ...
    if (!check_array_close(h_C, expected, M * N)) {
        throw std::runtime_error("2x2 matmul mismatch");
    }
}
...
TEST_CASE(matmul_4x3_3x5) {
    ...
    if (expected[0] != 46.0f) {
        throw std::runtime_error("CPU reference sanity check failed");
    }
    ...
    if (!check_array_close(h_C, expected, M * N)) {
        throw std::runtime_error("4x3 * 3x5 matmul mismatch");
    }
}

// Test 3: 64x128 * 128x64 — seeded random, compared against CPU reference
TEST_CASE(matmul_64x128_128x64) {
    ...
    if (!check_array_close(h_C.data(), expected.data(), size_C, 1e-3f, 1e-3f)) {
        throw std::runtime_error("64x128 * 128x64 matmul mismatch");
    }
}
...
TEST_CASE(matmul_lenet_fc1_256x120) {
    ...
    if (!check_array_close(h_C.data(), expected.data(), size_C, 1e-3f, 1e-3f)) {
        throw std::runtime_error("256x120 (LeNet FC1) matmul mismatch");
    }
}
