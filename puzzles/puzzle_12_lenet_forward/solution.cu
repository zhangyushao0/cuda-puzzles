// Puzzle 12: LeNet-5 Forward Pass (Full Network) — Reference Solution
//
// Chains all individual layer kernels from Puzzles 03-08 into a complete
// LeNet-5 forward pass:
//   Input(28x28x1) -> Conv1 -> ReLU -> Pool1 -> Conv2 -> ReLU -> Pool2
//   -> Flatten -> FC1 -> ReLU -> FC2 -> ReLU -> FC3 -> Softmax -> probs
//
// The student's job is ORCHESTRATION: allocating buffers, launching
// kernels in the right order with the right dimensions.
...
// (These are provided so Puzzle 12 is independently compilable)
// ============================================================
...
// ============================================================
// LeNet-5 parameter structure
// ============================================================

struct LeNetParams {
    // Conv1: 6 filters of 1x5x5
    float* conv1_w;   // [6 x 1 x 5 x 5] = 150
    float* conv1_b;   // [6]

    // Conv2: 16 filters of 6x5x5
...
struct LeNetActivations {
    ...
    // pool2_out doubles as flattened FC input (256 per sample)
...
// ============================================================
// Allocate / free activation buffers on GPU
// ============================================================
...
// ============================================================
// Allocate / free parameters on GPU
// ============================================================

void alloc_params(LeNetParams& p) {
    CUDA_CHECK(cudaMalloc(&p.conv1_w, 6   * 1 * 5 * 5 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.conv1_b, 6                * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.conv2_w, 16  * 6 * 5 * 5 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.conv2_b, 16               * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.fc1_w,   120 * 256        * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.fc1_b,   120              * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.fc2_w,   84  * 120        * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.fc2_b,   84               * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&p.fc3_w,   10  * 84         * sizeof(float)));
...
// ============================================================
// LeNet-5 Forward Pass — the core orchestration function
// ============================================================
...
void lenet_forward(const float* d_input, LeNetParams& p,
    ...
    // --- Conv1: (B, 1, 28, 28) -> (B, 6, 24, 24) ---
    {
        ...
        conv2d_forward<<<blocks, threads>>>(d_input, p.conv1_w, p.conv1_b,
                                             act.conv1_out,
                                             batch, 1, 28, 28, 6, 5);
        KERNEL_CHECK();
    }

    // --- ReLU1: (B, 6, 24, 24) -> (B, 6, 24, 24) ---
    {
        int n = batch * 6 * 24 * 24;
        int blocks = (n + threads - 1) / threads;
        relu_forward<<<blocks, threads>>>(act.conv1_out, act.relu1_out, n);
        KERNEL_CHECK();
    }
    ...
    // --- Pool1: (B, 6, 24, 24) -> (B, 6, 12, 12) ---
    {
        ...
        int blocks = (total + threads - 1) / threads;
        maxpool_forward<<<blocks, threads>>>(act.relu1_out, act.pool1_out,
                                              act.pool1_indices,
                                              batch, 6, 24, 24);
        KERNEL_CHECK();
    }
    ...
    // --- Conv2: (B, 6, 12, 12) -> (B, 16, 8, 8) ---
    {
        ...
        int blocks = (total + threads - 1) / threads;
        conv2d_forward<<<blocks, threads>>>(act.pool1_out, p.conv2_w, p.conv2_b,
                                             act.conv2_out,
                                             batch, 6, 12, 12, 16, 5);
        KERNEL_CHECK();
    }

    // --- ReLU2: (B, 16, 8, 8) -> (B, 16, 8, 8) ---
    {
        int n = batch * 16 * 8 * 8;
        int blocks = (n + threads - 1) / threads;
        relu_forward<<<blocks, threads>>>(act.conv2_out, act.relu2_out, n);
        KERNEL_CHECK();
    }
    ...
    // --- Pool2: (B, 16, 8, 8) -> (B, 16, 4, 4) ---
    {
        ...
        int blocks = (total + threads - 1) / threads;
        maxpool_forward<<<blocks, threads>>>(act.relu2_out, act.pool2_out,
                                              act.pool2_indices,
                                              batch, 16, 8, 8);
        KERNEL_CHECK();
    }

    // --- Flatten: (B, 16, 4, 4) -> (B, 256) --- [no kernel needed]
    // pool2_out is already contiguous: 16*4*4 = 256 per sample
    ...
    // --- ReLU3: (B, 120) -> (B, 120) ---
    {
        int n = batch * 120;
        int blocks = (n + threads - 1) / threads;
        relu_forward<<<blocks, threads>>>(act.fc1_out, act.relu3_out, n);
        KERNEL_CHECK();
    }
    ...
    // --- ReLU4: (B, 84) -> (B, 84) ---
    {
        int n = batch * 84;
        int blocks = (n + threads - 1) / threads;
        relu_forward<<<blocks, threads>>>(act.fc2_out, act.relu4_out, n);
        KERNEL_CHECK();
    }
    ...
    {
        dim3 block(16, 16);
        dim3 grid((10 + 15) / 16, (batch + 15) / 16);
    ...
    // --- Softmax: (B, 10) -> (B, 10) ---
    {
        ...
        softmax_forward<<<blocks, threads>>>(act.fc3_out, act.probs,
                                              batch, 10);
        KERNEL_CHECK();
    }
}
