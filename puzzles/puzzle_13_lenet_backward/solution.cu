// Puzzle 13: LeNet-5 Backward Pass (Full Backpropagation) — Reference Solution
//
// Chains all backward kernels from Puzzles 04-10 in reverse order to compute
// gradients for every learnable parameter in LeNet-5:
//   Softmax+CE_back -> FC3_back -> ReLU4_back -> FC2_back -> ReLU3_back
//   -> FC1_back -> Pool2_back -> ReLU2_back -> Conv2_back -> Pool1_back
//   -> ReLU1_back -> Conv1_back
//
// The student's job is REVERSE ORCHESTRATION: launching backward kernels
// in the correct reverse order with the correct dimensions and inputs.

#include <cuda_runtime.h>
#include <cfloat>

// ============================================================
// Bundled reference kernels from previous puzzles (forward)
...
// ============================================================
// Bundled reference kernels from previous puzzles (backward)
// ============================================================
...
// ============================================================
// LeNet-5 parameter and activation structures (from Puzzle 12)
// ============================================================
...
// Parameter gradient structure — same shape as LeNetParams
struct LeNetGrads {
    float* conv1_w;   // [6 x 1 x 5 x 5] = 150
    float* conv1_b;   // [6]
    float* conv2_w;   // [16 x 6 x 5 x 5] = 2400
...
// Intermediate gradient buffers for backward pass
struct LeNetGradActivations {
...
void alloc_grad_activations(LeNetGradActivations& ga, int batch) {
    CUDA_CHECK(cudaMalloc(&ga.grad_fc3_out,     batch * 10  * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_relu4_out,   batch * 84  * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_fc2_out,     batch * 84  * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_relu3_out,   batch * 120 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_fc1_out,     batch * 120 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_pool2_out,   batch * 256 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_relu2_out,   batch * 16 * 8 * 8 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_conv2_out,   batch * 16 * 8 * 8 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_pool1_out,   batch * 6  * 12 * 12 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_relu1_out,   batch * 6  * 24 * 24 * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&ga.grad_conv1_out,   batch * 6  * 24 * 24 * sizeof(float)));
}

void free_grad_activations(LeNetGradActivations& ga) {
    cudaFree(ga.grad_fc3_out);   cudaFree(ga.grad_relu4_out);
    cudaFree(ga.grad_fc2_out);   cudaFree(ga.grad_relu3_out);
    cudaFree(ga.grad_fc1_out);   cudaFree(ga.grad_pool2_out);
    cudaFree(ga.grad_relu2_out); cudaFree(ga.grad_conv2_out);
    cudaFree(ga.grad_pool1_out); cudaFree(ga.grad_relu1_out);
    cudaFree(ga.grad_conv1_out);
}

// ============================================================
// Forward pass (from Puzzle 12 solution)
// ============================================================
...
void lenet_forward(const float* d_input, LeNetParams& p,
    ...
    // Conv1: (B, 1, 28, 28) -> (B, 6, 24, 24)
    { int t = batch * 6 * 24 * 24; conv2d_forward<<<(t+threads-1)/threads, threads>>>(d_input, p.conv1_w, p.conv1_b, act.conv1_out, batch, 1, 28, 28, 6, 5); KERNEL_CHECK(); }
    // ReLU1
    { int n = batch * 6 * 24 * 24; relu_forward<<<(n+threads-1)/threads, threads>>>(act.conv1_out, act.relu1_out, n); KERNEL_CHECK(); }
    // Pool1: (B, 6, 24, 24) -> (B, 6, 12, 12)
    { int t = batch * 6 * 12 * 12; maxpool_forward<<<(t+threads-1)/threads, threads>>>(act.relu1_out, act.pool1_out, act.pool1_indices, batch, 6, 24, 24); KERNEL_CHECK(); }
    // Conv2: (B, 6, 12, 12) -> (B, 16, 8, 8)
    { int t = batch * 16 * 8 * 8; conv2d_forward<<<(t+threads-1)/threads, threads>>>(act.pool1_out, p.conv2_w, p.conv2_b, act.conv2_out, batch, 6, 12, 12, 16, 5); KERNEL_CHECK(); }
    // ReLU2
    { int n = batch * 16 * 8 * 8; relu_forward<<<(n+threads-1)/threads, threads>>>(act.conv2_out, act.relu2_out, n); KERNEL_CHECK(); }
    // Pool2: (B, 16, 8, 8) -> (B, 16, 4, 4)
    { int t = batch * 16 * 4 * 4; maxpool_forward<<<(t+threads-1)/threads, threads>>>(act.relu2_out, act.pool2_out, act.pool2_indices, batch, 16, 8, 8); KERNEL_CHECK(); }
    // FC1: (B, 256) -> (B, 120)
    { dim3 block(16, 16); dim3 grid((120+15)/16, (batch+15)/16); fc_forward<<<grid, block>>>(act.pool2_out, p.fc1_w, p.fc1_b, act.fc1_out, batch, 256, 120); KERNEL_CHECK(); }
    // ReLU3
    { int n = batch * 120; relu_forward<<<(n+threads-1)/threads, threads>>>(act.fc1_out, act.relu3_out, n); KERNEL_CHECK(); }
    // FC2: (B, 120) -> (B, 84)
    { dim3 block(16, 16); dim3 grid((84+15)/16, (batch+15)/16); fc_forward<<<grid, block>>>(act.relu3_out, p.fc2_w, p.fc2_b, act.fc2_out, batch, 120, 84); KERNEL_CHECK(); }
    // ReLU4
    { int n = batch * 84; relu_forward<<<(n+threads-1)/threads, threads>>>(act.fc2_out, act.relu4_out, n); KERNEL_CHECK(); }
    // FC3: (B, 84) -> (B, 10)
    { dim3 block(16, 16); dim3 grid((10+15)/16, (batch+15)/16); fc_forward<<<grid, block>>>(act.relu4_out, p.fc3_w, p.fc3_b, act.fc3_out, batch, 84, 10); KERNEL_CHECK(); }
    // Softmax
    { softmax_forward<<<(batch+threads-1)/threads, threads>>>(act.fc3_out, act.probs, batch, 10); KERNEL_CHECK(); }
}

// ============================================================
// LeNet-5 Backward Pass — the core backpropagation function
// ============================================================

void lenet_backward(const float* d_input, const float* d_labels,
                    LeNetParams& p, LeNetActivations& act,
                    LeNetGrads& grads, LeNetGradActivations& ga,
                    int batch) {
    const int threads = 256;

    // --- Step 1: Softmax + Cross-Entropy backward ---
    // ∂L/∂logits = probs - labels  [B x 10]
    {
        int blocks = (batch + threads - 1) / threads;
        softmax_ce_backward<<<blocks, threads>>>(act.probs, d_labels,
                                                  ga.grad_fc3_out,
                                                  batch, 10);
        KERNEL_CHECK();
    }

    // --- Step 2-4: FC3 backward ---
    // FC3: (B, 84) -> (B, 10), weights [10 x 84]
    // grad_fc3_w = grad_fc3_out^T * relu4_out
    {
        dim3 block(16, 16);
        dim3 grid((84 + 15) / 16, (10 + 15) / 16);
        fc_backward_weights<<<grid, block>>>(ga.grad_fc3_out, act.relu4_out,
                                              grads.fc3_w, batch, 84, 10);
        KERNEL_CHECK();
    }
    // grad_fc3_b = sum(grad_fc3_out, dim=batch)
    {
        int blocks = (10 + threads - 1) / threads;
        fc_backward_bias<<<blocks, threads>>>(ga.grad_fc3_out, grads.fc3_b,
                                               batch, 10);
        KERNEL_CHECK();
    }
    // grad_relu4_out = grad_fc3_out * fc3_w  [B x 84]
    {
        dim3 block(16, 16);
        dim3 grid((84 + 15) / 16, (batch + 15) / 16);
        fc_backward_input<<<grid, block>>>(ga.grad_fc3_out, p.fc3_w,
                                            ga.grad_relu4_out, batch, 84, 10);
        KERNEL_CHECK();
    }

    // --- Step 5: ReLU4 backward ---
    // grad_fc2_out = grad_relu4_out * (fc2_out > 0)  [B x 84]
    {
        int n = batch * 84;
        int blocks = (n + threads - 1) / threads;
        relu_backward<<<blocks, threads>>>(ga.grad_relu4_out, act.fc2_out,
                                            ga.grad_fc2_out, n);
        KERNEL_CHECK();
    }

    // --- Step 6-8: FC2 backward ---
    // FC2: (B, 120) -> (B, 84), weights [84 x 120]
    {
        dim3 block(16, 16);
        dim3 grid((120 + 15) / 16, (84 + 15) / 16);
        fc_backward_weights<<<grid, block>>>(ga.grad_fc2_out, act.relu3_out,
                                              grads.fc2_w, batch, 120, 84);
        KERNEL_CHECK();
    }
    {
        int blocks = (84 + threads - 1) / threads;
        fc_backward_bias<<<blocks, threads>>>(ga.grad_fc2_out, grads.fc2_b,
                                               batch, 84);
        KERNEL_CHECK();
    }
    // grad_relu3_out = grad_fc2_out * fc2_w  [B x 120]
    {
        dim3 block(16, 16);
        dim3 grid((120 + 15) / 16, (batch + 15) / 16);
        fc_backward_input<<<grid, block>>>(ga.grad_fc2_out, p.fc2_w,
                                            ga.grad_relu3_out, batch, 120, 84);
        KERNEL_CHECK();
    }

    // --- Step 9: ReLU3 backward ---
    // grad_fc1_out = grad_relu3_out * (fc1_out > 0)  [B x 120]
    {
        int n = batch * 120;
        int blocks = (n + threads - 1) / threads;
        relu_backward<<<blocks, threads>>>(ga.grad_relu3_out, act.fc1_out,
                                            ga.grad_fc1_out, n);
        KERNEL_CHECK();
    }

    // --- Step 10-12: FC1 backward ---
    // FC1: (B, 256) -> (B, 120), weights [120 x 256]
    {
        dim3 block(16, 16);
        dim3 grid((256 + 15) / 16, (120 + 15) / 16);
        fc_backward_weights<<<grid, block>>>(ga.grad_fc1_out, act.pool2_out,
                                              grads.fc1_w, batch, 256, 120);
        KERNEL_CHECK();
    }
    {
        int blocks = (120 + threads - 1) / threads;
        fc_backward_bias<<<blocks, threads>>>(ga.grad_fc1_out, grads.fc1_b,
                                               batch, 120);
        KERNEL_CHECK();
    }
    // grad_pool2_out = grad_fc1_out * fc1_w  [B x 256]
    {
        dim3 block(16, 16);
        dim3 grid((256 + 15) / 16, (batch + 15) / 16);
        fc_backward_input<<<grid, block>>>(ga.grad_fc1_out, p.fc1_w,
                                            ga.grad_pool2_out, batch, 256, 120);
        KERNEL_CHECK();
    }

    // --- Step 13: Pool2 backward ---
    // Pool2: (B, 16, 8, 8) -> (B, 16, 4, 4)
    // grad_relu2_out must be zero-initialized first
    {
        int n_relu2 = batch * 16 * 8 * 8;
        CUDA_CHECK(cudaMemset(ga.grad_relu2_out, 0, n_relu2 * sizeof(float)));
        int t = batch * 16 * 4 * 4;
        int blocks = (t + threads - 1) / threads;
        maxpool_backward<<<blocks, threads>>>(ga.grad_pool2_out,
                                               act.pool2_indices,
                                               ga.grad_relu2_out,
                                               batch, 16, 8, 8);
        KERNEL_CHECK();
    ...
    // --- Step 14: ReLU2 backward ---
    // grad_conv2_out = grad_relu2_out * (conv2_out > 0)  [B x 16 x 8 x 8]
    {
        int n = batch * 16 * 8 * 8;
        int blocks = (n + threads - 1) / threads;
        relu_backward<<<blocks, threads>>>(ga.grad_relu2_out, act.conv2_out,
                                            ga.grad_conv2_out, n);
        KERNEL_CHECK();
    }

    // --- Step 15-17: Conv2 backward ---
    // Conv2: (B, 6, 12, 12) -> (B, 16, 8, 8), filters [16 x 6 x 5 x 5]
    // Weight gradient
    {
        int t = 16 * 6 * 5 * 5;  // = 2400
        int blocks = (t + threads - 1) / threads;
        conv2d_backward_weights<<<blocks, threads>>>(ga.grad_conv2_out,
                                                      act.pool1_out,
                                                      grads.conv2_w,
                                                      batch, 6, 12, 12, 16, 5);
        KERNEL_CHECK();
    ...
    // Bias gradient
    {
        ...
        conv2d_backward_bias<<<blocks, threads>>>(ga.grad_conv2_out, grads.conv2_b,
                                                   batch, 16, 8, 8);
        KERNEL_CHECK();
    }
    // Input gradient -> grad_pool1_out  [B x 6 x 12 x 12]
    {
        int t = batch * 6 * 12 * 12;
        int blocks = (t + threads - 1) / threads;
        conv2d_backward_input<<<blocks, threads>>>(ga.grad_conv2_out, p.conv2_w,
                                                    ga.grad_pool1_out,
                                                    batch, 6, 12, 12, 16, 5);
        KERNEL_CHECK();
    }

    // --- Step 18: Pool1 backward ---
    // Pool1: (B, 6, 24, 24) -> (B, 6, 12, 12)
    // grad_relu1_out must be zero-initialized first
    {
        int n_relu1 = batch * 6 * 24 * 24;
        CUDA_CHECK(cudaMemset(ga.grad_relu1_out, 0, n_relu1 * sizeof(float)));
        int t = batch * 6 * 12 * 12;
        int blocks = (t + threads - 1) / threads;
        maxpool_backward<<<blocks, threads>>>(ga.grad_pool1_out,
                                               act.pool1_indices,
                                               ga.grad_relu1_out,
                                               batch, 6, 24, 24);
        KERNEL_CHECK();
    ...
    // --- Step 19: ReLU1 backward ---
    // grad_conv1_out = grad_relu1_out * (conv1_out > 0)  [B x 6 x 24 x 24]
    {
        int n = batch * 6 * 24 * 24;
        int blocks = (n + threads - 1) / threads;
        relu_backward<<<blocks, threads>>>(ga.grad_relu1_out, act.conv1_out,
                                            ga.grad_conv1_out, n);
        KERNEL_CHECK();
    }

    // --- Step 20-21: Conv1 backward ---
    // Conv1: (B, 1, 28, 28) -> (B, 6, 24, 24), filters [6 x 1 x 5 x 5]
    // Weight gradient
    {
        int t = 6 * 1 * 5 * 5;  // = 150
        int blocks = (t + threads - 1) / threads;
        conv2d_backward_weights<<<blocks, threads>>>(ga.grad_conv1_out,
                                                      d_input,
                                                      grads.conv1_w,
                                                      batch, 1, 28, 28, 6, 5);
        KERNEL_CHECK();
    ...
    // Bias gradient
    {
        int blocks = (6 + threads - 1) / threads;
        conv2d_backward_bias<<<blocks, threads>>>(ga.grad_conv1_out, grads.conv1_b,
                                                   batch, 6, 24, 24);
        KERNEL_CHECK();
    }
    // NOTE: We don't compute ∂L/∂input since the input is not a learnable parameter.
    // If needed for deeper networks, add conv2d_backward_input for Conv1 here.
}
